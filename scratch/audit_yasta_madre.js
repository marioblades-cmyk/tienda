...
async function main() {
    try {
        ...
        console.log(`--- BUSCANDO PAGOS "MADRE" (CON MOVIMIENTO DE CAJA) ---`);
        const pagos = await query(`cliente_pagos?vendedor_id=eq.${mauricioId}&created_at=gte.${sinceDate}&metodo_pago=eq.Yasta%20(QR)`);
        
        const madres = pagos.filter(p => !p.concepto.includes("Asignado a:") && !p.concepto.includes("Saldo recuperado"));
        
        madres.forEach(p => {
            console.log(`[${p.created_at}] BS ${p.monto} - ${p.concepto} (Caja ID: ${p.caja_mov_id})`);
        });
        
        console.log(`\nTotal Pagos Madre Yasta: ${madres.length}`);
